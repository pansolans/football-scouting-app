...
export const playerService = {
  ...
  getPlayersBatchInfo: async (playerIds: number[], playerNames: string[] = [], skipContract: boolean = false): Promise<Record<string, any>> => {
    const response = await api.post('/api/players/batch-info', { player_ids: playerIds, player_names: playerNames, skip_contract: skipContract });
    return response.data;
  },

  getCompetitionPlayers: async (competitionId: number, teamIds: number[] = []): Promise<any[]> => {
    const params: any = {};
    if (teamIds.length > 0) {
      params.team_ids = teamIds.join(',');
    }
    const response = await api.get(`/api/competitions/${competitionId}/players`, { params });
    return response.data;
  },
};
